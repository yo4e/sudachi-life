...
from sudachi_life.runtime_storage import ACTIVE_DATABASE_WAKE_RESERVE_BYTES
...
def test_enqueue_keeps_one_bounded_wake_of_active_database_headroom(
    initialized,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime_root, initial, _ = initialized
    ...
    connection = connect_database(paths.database, read_only=True)
    try:
        initial_pages = int(connection.execute("PRAGMA page_count").fetchone()[0])
        page_size = int(connection.execute("PRAGMA page_size").fetchone()[0])
    finally:
        connection.close()

    initial_bytes = initial_pages * page_size
    protected_limit = (
        initial_bytes + ACTIVE_DATABASE_WAKE_RESERVE_BYTES + (32 * page_size)
    )
    monkeypatch.setattr(
        "sudachi_life.runtime_storage.ACTIVE_DATABASE_MAX_BYTES",
        protected_limit,
    ...
    monkeypatch.setattr(
        "sudachi_life.lifecycle.ACTIVE_DATABASE_MAX_BYTES",
        protected_limit,
    )
    ...
    for index in range(1, 2000):
        candidate = f"wakeable-enqueue-{index:04d}-" + "x" * 100
        try:
            enqueue_garden_tick(
                paths,
                candidate,
                clock=FakeClock([ClockReading(2000 + index, index)]),
            )
        except InputRejectedError:
            rejected_id = candidate
            break
        accepted_ids.append(candidate)

    assert accepted_ids
    assert rejected_id is not None
    connection = connect_database(paths.database, read_only=True)
    try:
        allocated_before_wake = int(
    ...
    wake = perform_garden_wake(
        ...
        initial.organism_id,
        seed=1,
        clock=_wake_clock(5000),
    )
    assert wake.external_event_id == accepted_ids[0]
    assert wake.status == "sleeping"


def _leave_committed_prune_staging(
    initialized,
    monkeypatch: pytest.MonkeyPatch,
) -> tuple[Path, object, tuple[str, ...]]:
    runtime_root, initial, _ = initialized
    paths = OrganismPaths.build(runtime_root, initial.organism_id)
    for index in range(1, 4):
        _enqueue_and_wake(runtime_root, initial.organism_id, index)
    ...
    return runtime_root, initial, staging
...
def test_post_commit_retention_cleanup_is_explicit_and_reconcilable(
    ...
    runtime_root, initial, staging = _leave_committed_prune_staging(
        initialized,
        monkeypatch,
    )
    ...
    assert result.removed_staging_directories == staging
...
def test_retention_reconciliation_retries_after_delete_before_completion_audit(
    initialized,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime_root, initial, staging = _leave_committed_prune_staging(
    ...
    assert not any((paths.checkpoints / name).exists() for name in staging)
    connection = connect_database(paths.database, read_only=True)
    try:
        ...
        completed = connection.execute(
            """SELECT payload_json FROM event
               WHERE event_type = 'checkpoint_retention_cleanup_reconciled'"""
        ).fetchall()
        assert len(pending) == 1
        assert completed == []
        pending_sequence = int(pending[0]["event_sequence"])
    finally:
        connection.close()
    ...
    assert retry_clock.read_count == 0
    assert result.removed_staging_directories == staging
    assert result.remaining_staging_directories == ()
    assert result.audit_event_sequence is not None
    ...
    try:
        ...
        assert payload["reconciliation_pending_event_sequence"] == pending_sequence
        assert payload["removed_staging_directories"] == list(staging)
    finally:
        connection.close()
