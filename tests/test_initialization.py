...
def test_read_only_connection_handles_uri_characters(tmp_path, fixed_clock) -> None:
    runtime_root = tmp_path / "runtime?with#uri"
    status, _ = initialize_organism(runtime_root, "uri-safe", clock=fixed_clock)
    connection = connect_database(
        OrganismPaths.build(runtime_root, "uri-safe").database,
        read_only=True,
    )
    try:
        assert connection.execute("SELECT organism_id FROM organism").fetchone()[0] == status.organism_id
    finally:
        connection.close()
