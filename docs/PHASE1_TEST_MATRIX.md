...
Status: **Slices 1–19 implemented — Phase 1 incomplete**
...
| 2. Unexpected clock reads fail | `tests/test_clock.py::test_unexpected_clock_read_fails`; successful wakes consume five declared readings including the pre-action deadline check; the classified exhausted wake consumes four; maintenance inspection, JSONL export, rollback archive preparation, and restore-candidate construction consume no clock; rollback begin consumes one reading only after complete validation, while all protected rejection paths consume zero |
...
| 23. Sequence order is canonical | Genesis, canonical lifecycles, blocked-state, recovery, classified failures, maintenance, retention, repair, and rollback-start sequences are asserted; JSONL export is ordered only by canonical `event_sequence`; rollback begin requires its event at the exact next sequence; candidate construction requires that same event to remain the active tip |
...
| 26. Competing wake has one winner and one non-queued rejection | `tests/test_wake.py::test_competing_wake_is_rejected_and_not_queued`; rollback preparation, rollback begin, and restore-candidate construction also have protected fail-fast competing-writer rejection |
...
| 31. No later wake advances while checkpoint is pending | Pending state and repair rejection are protected; JSONL export, rollback preparation, rollback begin, and restore-candidate construction reject incompatible pending state without mutation; rollback-in-progress normal wakes reject before clock or claim |
| 32. Invalid checkpoint is not stable | Digest and directory-name mismatch tests |
| 33. Checkpoint validation covers protected identity and boundary | Genesis and lifecycle checkpoint validation, pending repair, JSONL export, rollback preparation, and rollback begin validate their declared protected checkpoint boundaries; `tests/test_rollback_candidate.py` additionally proves the published candidate is exactly equal to the immutable selected checkpoint for `user_version`, schema, every canonical row, and `sqlite_sequence` |
...
| 35. Retention is bounded and safe | Successful pruning and classified pruning failure are protected; pre-rollback archives and restore candidates remain outside checkpoint registry and ordinary retention |
| 36–38. Rollback archive, lineage, and failure recovery | Partial: Slice 17 proves source selection and complete verified pre-rollback archive publication. Slice 18 proves archive and active-body revalidation, exact canonical SQLite equality, atomic `rollback_in_progress` plus `rollback_started`, rejection paths, failure rollback, and normal-wake blocking. Slice 19 proves durable-intent revalidation, exact blocked-active-versus-archive equality, selected-source restoration through Online Backup, complete source-equal candidate validation, bounded atomic candidate publication, idempotence, and creation/publication failure cleanup with the active rollback state unchanged. Candidate lineage transformation, active replacement, completion history, and post-replacement recovery remain planned |
| 39. Protected authority cannot be modified by organism | Protected actions and evaluators are used; maintenance, repair, export, rollback preparation, rollback begin, and restore-candidate construction are explicit administrative boundaries; broader authority tests remain planned |
