...
Status: **Slices 1–5 implemented — Phase 1 incomplete**
...
| --- | --- |
| 1. Identical declared inputs produce identical canonical results | Partial: deterministic initialization and the canonical water, harvest, and completion-abstention policy are protected; full repeated-run equivalence remains planned |
...
| 5. One tick/observation/attempt/mutation maximum | water and harvest assert one attempt/mutation; completion abstention asserts zero attempts/mutations |
...
| 8. Hard-zero external capabilities | water, harvest, and completion-abstention ledgers assert zero caregiver, network, subprocess, and external-write consumption |
...
| 17. No negative counters | SQLite constraints and exact action/abstention ledgers cover the canonical three-wake path |
...
| 21. Failure streak and maintenance threshold | Completion abstention proves the justified zero/reset state; classified increments and threshold remain planned |
...
| 23. Sequence order is canonical | Genesis and all three canonical lifecycle event sequences are asserted |
...
| 30. Successful wake commits an exact pending boundary | Water validates 13, harvest 24, and completion abstention 34 |
...
| 33. Checkpoint validation covers protected identity and boundary | Genesis plus water, harvest, and abstention lifecycle checkpoint validation |
...
| 40. No organism-writable external workspace | SQLite-only action/abstention paths plus hard-zero ledgers cover the canonical run |
...
PR #19 passed GitHub Actions on Python 3.12 with **28 protected tests**.
