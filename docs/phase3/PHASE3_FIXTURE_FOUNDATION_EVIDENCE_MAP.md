...
| `P3-H01`–`P3-H12` | 12 | exhaustive declared substrate fields, digest/size/access checks, W1/W2 permission, technical caregiver-disablement proof | implemented for fixture substrate surface |
| `P3-I01`–`P3-I10` | 10 | negative attempt states and population visibility; `rolled_back` terminal evidence; frozen ADR 0007 runtime remains authoritative | implemented evidence shape + inherited rollback control; no new rollback capability |
| `P3-J01`–`P3-J12` | 12 | closed mandatory cost vector, measured/not-applicable/unmeasured semantics, monotonic cumulative accounting | implemented for fixture accounting |
| `P3-K01`–`P3-K14` | 14 | exact 14-group reviewed draft, external closure attestation, one bounded no-retry/no-edit seal | implemented for deterministic report package |
...
|---|---|---|
| `P3-B10`, `P3-J12` | exact attempt graph and incomplete-population rejection | `test_attempt_must_traverse_scheduled_started_terminal`; study validator |
...
| `P3-F06`–`P3-F10` | exact transition writers, terminal states, order, prerequisites, checkpoints, idempotent exact replay and same-ID conflict | transition validator; writer test; immutable replay test |
...
| `P3-K14` | reviewed draft → external closure → one bounded mechanical seal; no retry/edit | `test_publication_seal_cannot_retry_or_edit_semantics`; closure/seal validator |
...
| `P3IF-06` | four transitions are separate and ordered with exact writers/statuses | W3 invalid | transition validator + writer test |
| `P3IF-07` | caregiver-derived W1 substrate has complete current-episode provenance and matching digest/size | E2 invalid | substrate validator + permission/size tests |
...
| `P3IF-10` | acquisition requires valid E0 negative status and E1 pass; retention requires E2 pass with clean conformance | no retained-capability credit | capability validator + positive fixture |
| `P3IF-11` | capabilities protected at E0 cannot silently regress | W3 invalid | protected-regression test |
| `P3IF-12` | study population has exact planned ordinals and terminal state history | study/W3 invalid | study validator + attempt graph test |
...
| `P3IF-14` | measured cumulative cost cannot decrease | cost/W3 invalid | cost monotonic validator |
| `P3IF-15` | Stage-1 draft has exactly fourteen groups in accepted order | report/W3 invalid | exact-group test |
...
| `P3IF-19` | deterministic fixture report cannot claim maturity/developmental effectiveness/novelty | package invalid for fixture scope | positive report limitations + ADR 0018 |
...
1. create one reviewable PR from `feat/phase3-withheld-caregiver-foundation`;
2. pass ordinary GitHub Actions with the complete pre-existing protected suite plus the new Phase 3 tests;
3. verify Phase 1 original protected blobs and Phase 2 accepted evidence remain intact through the existing controls;
4. record one exact candidate head and CI run;
5. obtain one independent read-only implementation audit under `docs/CODEX_INDEPENDENT_AUDIT_POLICY.md`;
6. repair accepted findings without weakening the contract or tests;
7. synchronize this map, `docs/HANDOFF.md`, Issue #145, and the PR before freeze/merge completion.
